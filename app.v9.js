...
const DRAG_THRESHOLD_PX = 5; // Minimum pixels before drag starts
...
function CharacterMapper() {
  ...
  const applyGroupStep = (stepX, stepY, anchorId) => {
    ...
    setData(prev => {
      const group =
        selectedIds.size && selectedIds.has(anchorId)
          ? selectedIds
          : new Set([anchorId]);
      return {
        ...prev,
  ...
  const savePublishCfg = (cfg) => {
    setPublishCfg(cfg);
    try {
      localStorage.setItem('ghPublishConfig', JSON.stringify(cfg));
    } catch {}
  };
  ...
  const loadFromRemoteJson = async () => {
    try {
      ...
      if (resp.ok) {
        ...
        console.log('[v8] Loaded data.json from server');
      } else {
        console.log('[v8] data.json not found or not ok:', resp.status);
      }
    } catch (e) {
      console.warn('[v8] Failed to fetch data.json', e);
    }
  };
  ...
  const publishToGitHub = async (message = 'Update data.json from app') => {
    ...
    try {
      ...
      console.log('[v8] Published to GitHub successfully.');
  ...
  const normalizeData = (d) => {
    ...
    if (!cloned.legend) cloned.legend = { lines: {}, symbols: {}, sects: {}, statusSymbols: {} };
  ...
  const saveToStorage = (payload = data) => {
    try {
      ...
      console.log('[v8] Saved to localStorage');
    } catch (e) {
  ...
  const loadFromStorage = () => {
    try {
      ...
      if (raw) {
        ...
        console.log('[v8] Loaded saved data from localStorage');
      }
    } catch (e) {
            ...
            data.characters.map((char) => {
              ...
              return React.createElement(CharacterTile, {
                ...
                isSelected: selectedIds.has(char.id) || 
                  (!selectedIds.size && selectedCharacter && selectedCharacter.id === char.id),
                onPrimarySelect: () => {
                  setOnlySelected(char.id);
                  setSelectedCharacter(char);
                  if (!isEditing) setActiveTab('characters');
                },
                onToggleSelect: () => {
